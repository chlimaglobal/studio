...
// ajuste de tipo: categorias que o Set de investimentos realmente aceita
  ...
  useEffect(() => {
    if (transactions.length > 0) {
      const totalInvested = transactions
        .filter((t) => allInvestmentCategories.has(t.category as InvestmentCategory))
        .reduce((acc, t) => {
          if (t.type === 'income') return acc + t.amount;
          // Treat withdrawals as negative contributions to patrimony
          if (t.type === 'expense' && investmentWithdrawalCategories.has(t.category as InvestmentCategory)) {
            return acc - t.amount;
          }
  ...
  const {
    ...
    if (investmentYears <= 0)
      return {
        ...
        totalNestEggNeeded: 0,
        projectedPatrimony: 0,
        difference: 0,
      };

    // 1. Calculate the future value of the desired income due to inflation
    const futureDesiredIncome =
      desiredRetirementIncome * Math.pow(1 + inflationRate / 100, investmentYears);
    ...
    const totalNestEggNeeded =
      retirementDuration > 0
        ? (futureDesiredIncome / monthlyRealYield) *
          (1 - Math.pow(1 + monthlyRealYield, -retirementDuration * 12))
        : 0;

    // 3. Project the growth of current savings + future contributions
    let accumulated = currentPatrimony;
    const projData = [
      { year: 0, patrimony: currentPatrimony, goal: totalNestEggNeeded },
    ];
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Planeje o futuro financeiro do casal.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Users className="h-5 w-5" /> Dados do Casal
              </CardTitle>
            </CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDesiredRetirementIncome(Number(e.target.value))
                  }
                />
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  step="0.1"
                  value={realYield}
                  onChange={(e) => setRealYield(Number(e.target.value))}
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Retorno esperado acima da inflação.
                </p>
              </div>
              <div>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          >
            <CardHeader>
              <CardTitle
                className={isOnTrack ? 'text-green-600' : 'text-destructive'}
              >
                {isOnTrack
              ...
              <CardDescription>
                Com base nos seus aportes atuais, seu patrimônio projetado na
                aposentadoria é de {formatCurrency(projectedPatrimony)}. Sua
                meta de patrimônio é de {formatCurrency(totalNestEggNeeded)}. A
                diferença é de{' '}
                <span
                  className={
                    isOnTrack
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <LineChart className="h-5 w-5" /> Projeção de Patrimônio
              </CardTitle>
              <CardDescription>
                A evolução do seu dinheiro ao longo do tempo.
              </CardDescription>
            </CardHeader>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <RechartsLineChart data={projectionData}>
                  ...
                  <XAxis
                    dataKey="year"
                    fontSize={12}
                    tickFormatter={(val) => `${new Date().getFullYear() + val}`}
                  />
                  <YAxis
                    fontSize={12}
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">
                  {formatCurrency(desiredRetirementIncome)} /mês
                </span>
              </div>
              <Separator />
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">
                  Renda desejada corrigida
                </span>
                <span className="font-semibold">
                  {formatCurrency(futureDesiredIncome)} /mês
                </span>
              </div>
              <Separator />
              <div className="flex justify-between items-center">
