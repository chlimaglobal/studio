...
async function callFirebaseFunction<I, O>(functionName: string, data: I): Promise<O> {
    try {
        ...
        console.error(
            `Error calling Firebase function '${functionName}':`,
            error.code,
            error.message
        );
        ...
        /**
         * ✅ AJUSTE PRINCIPAL:
         * permission-denied NÃO é erro técnico,
         * é regra de negócio (feature premium).
         * Portanto, NÃO lançamos exceção.
         */
        if (error.code === 'functions/permission-denied') {
            return {
                __premiumRequired: true
            } as O;
        }
        ...
        if (error.code === 'functions/not-found') {
            console.error(
                `[DEBUG] Função '${functionName}' não encontrada — verifique o deploy com 'firebase deploy --only functions'`
            );
            throw new Error(
                `A função '${functionName}' não foi encontrada no backend. Verifique se ela foi implantada corretamente.`
            );
        }
        
        throw new Error(
            error.message || `Falha ao executar ${functionName}: Ocorreu um erro desconhecido.`
        );
    }
}
...
export async function getCategorySuggestion(input: CategorizeTransactionInput): Promise<CategorizeTransactionOutput> {
    return callFirebaseFunction<CategorizeTransactionInput, CategorizeTransactionOutput>(
...
export async function extractTransactionInfoFromText(input: ExtractTransactionInput): Promise<ExtractTransactionOutput> {
    return callFirebaseFunction<ExtractTransactionInput, ExtractTransactionOutput>(
        'extractTransactionInfoFromText',
        input
    );
}
...
export async function extractMultipleTransactions(input: ExtractMultipleTransactionsInput): Promise<ExtractMultipleTransactionsOutput> {
    return callFirebaseFunction<ExtractMultipleTransactionsInput, ExtractMultipleTransactionsOutput>(
        'extractMultipleTransactions',
        input
    );
}

export async function runAnalysis(input: GenerateFinancialAnalysisInput): Promise<GenerateFinancialAnalysisOutput> {
    return callFirebaseFunction<GenerateFinancialAnalysisInput, GenerateFinancialAnalysisOutput>(
        'runAnalysis',
        input
    );
}
...
export async function runFileExtraction(input: ExtractFromFileInput): Promise<ExtractFromFileOutput> {
    return callFirebaseFunction<ExtractFromFileInput, ExtractFromFileOutput>(
        'runFileExtraction',
        input
    );
}
...
export async function runInvestorProfileAnalysis(input: InvestorProfileInput): Promise<InvestorProfileOutput> {
    return callFirebaseFunction<InvestorProfileInput, InvestorProfileOutput>(
        'runInvestorProfileAnalysis',
        input
    );
}

export async function runSavingsGoalCalculation(input: SavingsGoalInput): Promise<SavingsGoalOutput> {
    return callFirebaseFunction<SavingsGoalInput, SavingsGoalOutput>(
        'runSavingsGoalCalculation',
        input
    );
}

export async function runGoalMediation(input: MediateGoalsInput): Promise<MediateGoalsOutput> {
    return callFirebaseFunction<MediateGoalsInput, MediateGoalsOutput>(
        'runGoalMediation',
        input
    );
}
...
export async function runImageExtraction(input: ExtractFromImageInput): Promise<ExtractFromImageOutput> {
    return callFirebaseFunction<ExtractFromImageInput, ExtractFromImageOutput>(
        'runImageExtraction',
        input
    );
}
