...
import { EditGoalFormSchema, Goal } from '@/types';
import { useToast } from '@/hooks/use-toast';
import { z } from 'zod';
import { ScrollArea } from './ui/scroll-area';
import Icon from './icon';
import { updateStoredGoal } from '@/lib/storage';
...
export function EditGoalDialog({ goal, open, onOpenChange }: EditGoalDialogProps) {
  ...
  const form = useForm<z.infer<typeof EditGoalFormSchema>>({
    ...
    defaultValues: {
      ...goal,
      targetAmount: formatAmountForInput(goal.targetAmount),
      currentAmount: formatAmountForInput(goal.currentAmount),
  ...
  async function onSubmit(values: z.infer<typeof EditGoalFormSchema>) {
    if (!user) {
        toast({
            variant: 'destructive',
            title: 'Erro de Autenticação',
            description: "Você precisa estar logado para editar uma meta."
        });
        return;
    }
    try {
      // @ts-ignore - Schema conversion is handled
      await updateStoredGoal(user.uid, goal.id, values);
      toast({
          title: 'Sucesso!',
